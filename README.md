...
|----------|---------|--------------|
| **Win-rate anomaly** | cheating / bug exploits | Every game has a known RTP and return variance (from its paytable and designed edge). Given how many bets an account has placed, HouseWatch z-scores its realised return against that. Sitting 6+ sigma above expected isn't luck. |
...
The win-rate detector is the interesting one. Because each game's expected return
and variance are known, HouseWatch knows exactly how an honest player's results
...
housewatch/
  housewatch/
    ...
    games.py             per-game RTP + return std (each game's known odds)
